...
using rsr.Max.Exceptions;

namespace rsr.Max.Domain.Entities
{
    ...
    public class Order : AuditableEntity
    {
        ...
        [Comment("Public unique identifier")] public Guid OrderResourceId { get; private set; }
        ...
        {
            Guard.Against(() => Status == OrderStatus.Cancelled || Status == OrderStatus.Shipped,
                () => throw new InvalidOrderStateChangeMessageException($"Update not allowed when Status is {Status}"));
        }
        ...
        {
            AssertOpenOrder();
            Guard.Against(() => item == null || !items.Contains(item),
                () => throw new InvalidItemMessageException("Item to remove must not be null and must be part of order"));
        ...
        {
            AssertOpenOrder();
            Guard.Against(() => itemsToRemove == null || itemsToRemove.Count == 0,
                () => throw new InvalidItemMessageException("Items to remove must not be null and have items"));
